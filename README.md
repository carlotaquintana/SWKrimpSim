...
## Running the FP-Tree Builder (FPTreeBuilder.jar)

Builds a graph from a dataset's code table and exports it as JSON for the D3.js viewer. Requires the files generated by the SLIM Docker step (see above).

### Prerequisites

These files must exist under `results/<database_name>-output/`:

- `ct-latest.ct`
- `<database_name>.db.analysis.txt`
- `conversionIndex.idx`

### Usage

```bash
java -jar FPTreeBuilder.jar <dataset> [selectedItem] [--sorted]
```

- `dataset`: dataset name, matching the `results/<dataset>-output/` folder.
- `selectedItem` (optional): a Vreeken item code. If given, only patterns containing it are used, and usage is normalized against its own support. If omitted, the whole code table is used (no filtering) and usage is normalized against the total usage of the code table.
- `--sorted` (optional): reorders the items of every pattern before building the graph, placing TYPE items first and FINISH 

selectedItem and --sorted can be combined and given in any order.

Examples:

```bash
java -jar FPTreeBuilder.jar WktEN 908
java -jar FPTreeBuilder.jar WktEN
java -jar FPTreeBuilder.jar WktEN --sorted
java -jar FPTreeBuilder.jar WktEN 908 --sorted
```
...
```text
results/<database_name>-output/fptree-<selectedItem|all>[-sorted].json
```

Contains:

- `nodes`: blocks of items that always co-occur together, with their Vreeken/original ids, usage, and the `.ct` patterns they appear in.
- `links`: co-occurrence relations between blocks.
